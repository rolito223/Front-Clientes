...
class TabContentIcon(ft.UserControl):
    ...
    def update_icon(self, e: ft.ControlEvent):
        ...
        self.icon_size = int(self.field_size.value.strip(
        )) if self.field_size.value.strip().isnumeric() else 65

        self.icon_color = self.field_color.value.strip(
        ) if self.field_color.value.strip() else None
        self.icon_name = self.field_name.value.strip(
        ) if self.field_name.value.strip() else "cake_rounded"
        self.icon_tooltip = self.field_tooltip.value.strip(
        ) if self.field_tooltip.value.strip() else None

        self.icon_offset = self.field_offset.value.strip(
        ) if self.field_offset.value.strip() else "None"
        self.icon_rotate = self.field_rotate.value.strip(
        ) if self.field_rotate.value.strip() else "None"
        self.icon_scale = self.field_scale.value.strip(
        ) if self.field_scale.value.strip() else "None"

        # name
        try:
            if self.icon_name is not None:
                self.icon_name = eval(
                    self.icon_name) if '.' in self.icon_name else self.icon_name.lower()
        ...
        try:
            if self.icon_color is not None:
                self.icon_color = eval(
                    self.icon_color) if '.' in self.icon_color else self.icon_color.lower()
                ...
                if self.icon_color not in all_flet_colors:
                    raise ValueError(
                        "Entered color was not found! See the colors browser for help!")
        except Exception as x:
            print(f"Color Error: {x}")
            e.page.show_snack_bar(ft.SnackBar(
                ft.Text(f"ERROR: {x}"), open=True))
            return
        ...
        try:
            ...
            if not isinstance(self.icon_offset, ft.Offset) \
                ...
                self.icon_offset = eval(
                    f"Offset({self.icon_offset[0]}, {self.icon_offset[1]})")
        except Exception as x:
        ...
        try:
            ...
            def deg_to_rads(d): return round((math.pi * float(d)) / 180, 3)
            if not isinstance(self.icon_rotate, ft.Rotate) \
                ...
                self.icon_rotate = eval(
                    f"Rotate({deg_to_rads(self.icon_rotate[0])}, {self.icon_rotate[1]})")
        except Exception as x:
        ...
        try:
            if self.field_opacity.value:
                ...
                assert isinstance(self.icon_opacity, (int, float)
                                  ), "`opacity` must be either of type float or int !"
            else:
                self.icon_opacity = None
        except Exception as x:
    ...
    def copy_to_clipboard(self, e: ft.ControlEvent):
        ...
        e.page.show_snack_bar(ft.SnackBar(
            ft.Text(f"Copied: {val}"), open=True))
        print(val)
